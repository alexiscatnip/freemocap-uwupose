...
class recordGUI:
    ...
    def __init__(self, master):
        ...
        tk.Label(self.master, text="Select Your Cameras",
        ...
        tk.Label(
            self.master,
            text="""Select 'Setup' to preview cameras \n or \n 'Record' to create recording""",
            justify=tk.CENTER
        ).pack()

        for option, val in options:
            tk.Radiobutton(
                self.master, text=option, padx=20, variable=self.currentTask,
                value=val
            ).pack(anchor=tk.W)

        self.refresh_button = Button(bottom_frame, text="Re-detect Cameras",
                                     command=self.Refresh)
        self.refresh_button.pack(side=tk.RIGHT)

        self.sub_button = Button(bottom_frame, text="Submit",
                                 command=self.Submit)
...
class SettingsGUI:
    def __init__(
        ...
        rotationValues = self.RotationRetrieval(cam_inputs,
                                                rotation_dictionary)
        ...
        for cam, rotDegree in zip(self.cam_inputs, rotationValues):
            ...
            tk.Label(rotLabelFrame,
                     text="Choose rotation for Cam " + str(cam)).pack()
            for option, degree in rotation_options:
                rotationRadioButton[cam] = tk.Radiobutton(
                    rotLabelFrame, text=option, value=degree,
                    variable=current_rotation
                ).pack()
        ...
        heightLabel = Label(resFrame, text="Resolution Height").pack(
            side="left")
        self.resHeightEntry = Entry(resFrame)
        self.resHeightEntry.pack(side="left")

        # ---Exposure,FPS, and codec entry
        exposureLabel = Label(parametersFrame, text="Exposure").pack(
            side="left")
        ...
        if self.task == "setup":
            self.var1 = tk.IntVar()
            c1 = tk.Checkbutton(parametersFrame, text='Mediapipe Overlay',
        ...
        self.sub_button = Button(bottomFrame, text="Submit - Start Cameras",
                                 command=self.Submit)
...
class ProceedToRecordGUI:
    def __init__(self, master):
        ...
        sessionText = "Proceed to Work? (Please have steamVR + headset ready and connected)"
        sessionLabel = Label(master, text=sessionText)
        sessionLabel.pack(side="top")

        # master.title("Proceed to Recording?")
        self.proceed_button = Button(text="Begin Working!",
                                     command=self.proceed)
        self.change_button = Button(text="Change Parameters",
                                    command=self.change_params)
...
def RunParametersGUI(rotation_entry, parameter_entry, cam_inputs, task):
    # ---Get all the necessary parameters
    root = tk.Tk()
    recording_settings = SettingsGUI(
        root, cam_inputs, parameter_entry, rotation_entry, task
    )
    root.mainloop()

    return recording_settings.rotation_output, \
           recording_settings.paramDict, \
           recording_settings.mediaPipeOverlay


def RunProceedtoRecordGUI():
    root = tk.Tk()
    goToRecording = ProceedToRecordGUI(root)
    root.mainloop()

    return goToRecording.continueToRecording, goToRecording.restart_setup
